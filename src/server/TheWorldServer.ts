...
export class TheWorldServer {
  ...
  private setupRoutes() {
    ...
    this.app.get("/api/status", async (req: Request, res: Response) => {
      try {
        const regions = this.regionManager
          ? await this.regionManager.listRegions()
          : [];
        res.json({
          status: "ok",
    ...
    this.app.post("/api/ai/exec", async (req: Request, res: Response) => {
      try {
        ...
        if (!ai || !region || !command) {
          return res
            .status(400)
            .json({ error: "AI name, region, and command are required" });
        }
    ...
    this.app.post(
      "/api/agent/:region/:user/serve/start",
      async (req: Request, res: Response) => {
        try {
          const { region, user } = req.params;
          const { port } = req.body;
    ...
    this.app.post(
      "/api/agent/:region/:user/serve/stop",
      async (req: Request, res: Response) => {
        try {
          const { region, user } = req.params;
    ...
    this.app.post("/api/oracle/send", async (req: Request, res: Response) => {
      try {
        ...
        if (!to || !region || !message) {
          return res
            .status(400)
            .json({ error: "Target AI, region, and message are required" });
        }
          ...
          const proxy = createProxyMiddleware({
            ...
            on: {
              error: (err: any, req: Request, res: any) => {
                logger.error(
                  { error: err, region, port },
                  "OpenCode proxy error",
                );
                if (!res.headersSent) {
                  res
                    .status(502)
                    .json({ error: "Failed to connect to OpenCode instance" });
                }
              },
            },
          });
