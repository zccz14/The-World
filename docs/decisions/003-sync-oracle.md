...
**历史阶段决策（v0.1）** - 已被 v0.2 异步事件驱动模型替代

## 迁移说明（2026-03-10）

本 ADR 记录了 v0.1 MVP 阶段选择同步调用的理由，该决策在当时是合理的工程权衡。

**v0.2 迁移方向**：

- 从同步 HTTP 调用迁移到异步事件驱动（inbox/outbox）
- 核心原因：任务本质是宽泛/重型，且可能多次返回、持续对话；同步 `req/res` 语义天然不匹配
- Agent 拥有回复主权：自主决定何时/是否/如何多次回复人类
- 价值排序：可靠投递、可恢复、可审计 > 低延迟

详见 `docs/02-current-arch.md` 的"设计意图与价值判断"章节。
